...
;; Simple format
;; HEXFONT0
;; (repeat (3 byte range-start, where highest nibble is row-type, inclusive)
;;         (3 byte range-end, exclusive))
;; (000030 ends the ranges with invalid 30 byte)
;; (000000 as padding)
    ...
    (call-with-output-file (string-append filename ".bin")
      (lambda (bin)
        ...
        (let* ((number-bv (make-bytevector 32))
                ...
                (call-with-output-bytevector
                 (lambda (content-port)
                   (do ((line (get-line hex) (get-line hex))
                       ...
                       ((eof-object? line)
                        (bytevector-uint-set! number-bv 0 (logior (row-type->hinibble row-type) range-start) (endianness little) 3)
                        (bytevector-uint-set! number-bv 3 range-end (endianness little) 3)
                        (put-bytevector bin number-bv 0 6))
          ...
          ;; The header ends with (little-endian) value #x000030, which is invalid for any range-start value
          (bytevector-uint-set! number-bv 0 #x300000 (endianness little) 3)
          (put-bytevector bin number-bv 0 3)
